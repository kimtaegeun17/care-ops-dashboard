...
export function exportFullReport(
  ...
  // 비정상장비 - no address column
  ws1Data.push(['비', '장비이상', '대상자명', '생년월일', '핸드폰번호', 'G/W번호', '차수', 'G/W AS', '', '시작시각', '지속시간']);
  abnormalDevice.forEach((c) => {
    ws1Data.push(['비', c.deviceTag || '', c.person.name, c.person.birthDate, c.person.phone, c.person.gwNumber || '', c.person.order || '', c.gwAs || '', '', c.detectedTime, c.elapsedTime]);
  });

  const ws1 = XLSX.utils.aoa_to_sheet(ws1Data);
  ws1['!cols'] = [
    { wch: 3 }, { wch: 18 }, { wch: 8 }, { wch: 10 }, { wch: 30 },
