...
export default function OutingScreen() {
  ...
  const cases = filterByGwAs(filtered.longOuting, gwAs);

  return (
    <div className="p-6 animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold text-foreground mb-1">장기외출</h2>
          <p className="text-sm text-muted-foreground">{cases.length}건</p>
        </div>
        <GwAsFilter cases={filtered.longOuting} value={gwAs} onChange={setGwAs} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-table-header text-table-header-foreground">
              {['', '대상자명', '출생년도', '도로명주소', '핸드폰번호', '게이트웨이번호', '차수', 'G/W AS', '외출시작', '지속시간', '조치방법', '결과', '비고'].map(h => (
          ...
          <tbody className="divide-y divide-border">
            {cases.map((c, i) => (
              <CaseRow key={c.id} c={c} index={i + 1} updateCase={(id, u) => updateCase('longOuting', id, u)} />
            ))}
            {cases.length === 0 && (
...
function CaseRow({ c, index, updateCase }: { c: CaseRecord; index: number; updateCase: (id: string, u: Partial<CaseRecord>) => void }) {
  return (
    <tr className={`${index % 2 === 0 ? 'bg-table-row-alt' : 'bg-card'} hover:bg-table-row-hover transition-colors`}>
      ...
      <td className="px-2 py-2 text-xs font-medium text-warning whitespace-nowrap">{c.elapsedTime}</td>
