...
export default function DeviceScreen() {
  ...
  const cases = filtered.abnormalDevice;
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-table-header text-table-header-foreground">
              {['비', '장비이상', '대상자명', '생년월일', '도로명주소', '핸드폰번호', '게이트웨이번호', '차수', '시작시각', '지속시간', '조치방법', '결과', '비고'].map(h => (
...
function CaseRow({ c, index, updateCase }: { c: CaseRecord; index: number; updateCase: (id: string, u: Partial<CaseRecord>) => void }) {
  return (
    <tr className={`${index % 2 === 0 ? 'bg-table-row-alt' : 'bg-card'} hover:bg-table-row-hover transition-colors`}>
      ...
      <td className="px-2 py-2 text-xs text-foreground max-w-[150px] truncate">{c.person.address}</td>
